...
def _TransformBackground(bg: Image.Image) -> Image.Image:
    ...
    # mild rotation (corners filled — masked by the zoom-crop below)
    angle: float = random.uniform(a=-6.0, b=6.0)
    ...
    # zoom-in (also masks rotation corners when present)
    zoom_min: float = 1.06 if rotated_applied else 1.00
    zoom: float = random.uniform(a=zoom_min, b=1.18)
    if zoom > 1.01:
    ...
    # tone (mild — task is shape-based so safe for color-blind users)
    bg = ImageEnhance.Brightness(image=bg).enhance(factor=random.uniform(a=0.90, b=1.10))
    bg = ImageEnhance.Contrast(image=bg).enhance(factor=random.uniform(a=0.90, b=1.20))
    bg = ImageEnhance.Color(image=bg).enhance(factor=random.uniform(a=0.60, b=1.20))
    bg = ImageEnhance.Sharpness(image=bg).enhance(factor=random.uniform(a=0.80, b=1.40))
